...
import { updateCountProduct } from '../../updatecountcart'
import EmptyCartPage from '../empty/empty'
...
export default function Cart() {
  ...
  {
    isLoading ? <LoadingScreen/> : cartItems?.numOfCartItems ?<section className="py-24 relative">
<div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
    ...
    </div>
    {
      cartItems?.data.products.map((item,index)=>{
        ...
        return <div key={index} className="grid grid-cols-1 lg:grid-cols-2 min-[550px]:gap-6 border-t border-gray-200 py-6">
        <div
            ...
            <div className="img-box"><img src={item.product.imageCover} alt="perfume bottle image" className="xl:w-[140px] rounded-xl"/></div>
            <div className="pro-data w-full max-w-sm ">
                <h5 className="font-semibold text-xl leading-8 text-black max-[550px]:text-center">{item.product.title}</h5>
                <p
        ...
        <div
            ...
            <h6 className="font-manrope font-bold text-2xl leading-9 text-black w-full max-w-[176px] text-center">
                $0.00 <span className="text-sm text-gray-300 ml-3 lg:hidden whitespace-nowrap">(Delivery
                    Charge)</span></h6>
            <div className="flex items-center w-full mx-auto justify-center">
                <button onClick={()=>{checkCount(item.product._id,item?.count)
                }}
                ...
                <input type="text" onChange={(e)=>{e.target.value>0 ? addToCart(item.product._id):removeFromCart(item?.product._id)}}
            ...
            <h6
                ...
                ${item.count*item.price}</h6>
                <span onClick={()=>{removeFromCart(item?.product._id)}} className="hover:text-red-700 cursor-pointer font-manrope font-medium text-2xl leading-9 text-gray-900"><i class="fa-solid fa-xmark"></i></span>
        </div>
        </div>
      })
    }
    <div className="bg-gray-50 rounded-xl p-6 w-full mb-8 max-lg:max-w-xl max-lg:mx-auto">
        <div className="flex items-center justify-between w-full mb-6">
            ...
            <h6 className="font-semibold text-xl leading-8 text-gray-900">${cartItems?.data.totalCartPrice}</h6>
        </div>
        <div className="flex items-center justify-between w-full pb-6 border-b border-gray-200">
            <p className="font-normal text-xl leading-8 text-gray-400">Delivery Charge</p>
            <h6 className="font-semibold text-xl leading-8 text-gray-900">$0.00</h6>
        </div>
        <div className="flex items-center justify-between w-full py-6">
            <p className="font-manrope font-medium text-2xl leading-9 text-gray-900">Total</p>
            <div className="flex items-center justify-between w-1/4 py-6">
                <p className="font-manrope font-medium text-2xl leading-9 text-gray-900">{countProducts}</p>
                <h6 className="font-manrope font-medium text-2xl leading-9 text-indigo-500">${cartItems?.data.totalCartPrice}</h6>
    ...
    <div className="flex items-center flex-col sm:flex-row justify-center gap-3 mt-8">
        <button
            ...
            <Link to={'/home'} className="px-2 font-semibold text-lg leading-8 text-indigo-600">Continue Shoping</Link>
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                <path d="M8.25324 5.49609L13.7535 10.9963L8.25 16.4998" stroke="#4F46E5" strokeWidth="1.6"
                    strokeLinecap="round" strokeLinejoin="round" />
            </svg>
        </button>
        <Link to={'/checkout/'+cartItems.cartId}
